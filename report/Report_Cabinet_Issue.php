...
$pdf->SetTitle('Report_Cabinet_Issue_(IOR)');
...
if($type_date == 1){
    ...
    if($checkday == 1){
        $date1 = explode("-", $date1);
        $text_date = "วันที่หยิบอุปกรณ์ : " . $date1[0] . " " . $datetime->getTHmonthFromnum($date1[1]) . " " . " พ.ศ." . " " .($date1[2] + 543 );
    }else{
        $date1 = explode("-", $date1);
        $date2 = explode("-", $date2);

        $text_date = "วันที่หยิบอุปกรณ์ : " . $date1[0] . " " . $datetime->getTHmonthFromnum($date1[1]) . " " . " พ.ศ." . " " .($date1[2] + 543 ) . " ถึง " .  $date2[0] . " " . $datetime->getTHmonthFromnum($date2[1]) . " " . " พ.ศ." . " " .($date2[2] + 543 );
    }
}
...
if($type_date == 2){
    ...
    if($checkmonth == 1){
        $text_date = "เดือนที่หยิบอุปกรณ์ : " . $datetime->getTHmonthFromnum($month1);
    }else{
        $text_date = "เดือนที่หยิบอุปกรณ์ : " . $datetime->getTHmonthFromnum($month1) . " ถึง " . $datetime->getTHmonthFromnum($month2);
    }
}
    ...
    $pdf->Cell(0, 10,  "รายงานหยิบอุปกรณ์ประจำวันตู้ RFID SmartCabinet", 0, 1, 'C');
...
<th width="15 %" align="center">ผู้ปฏิบัติงาน</th>
<th width="15 %" align="center">HN CODE</th>
</tr> </thead>';
...
$ddate = date('d_m_Y');
$pdf->Output('Report_Cabinet_Issue_(IOR)_' . $ddate . '.pdf', 'I');
