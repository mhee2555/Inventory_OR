...
$pdf->SetTitle('Report_log_cabinet');
...
if ($type_date == 1) {
    ...
    if ($checkday == 1) {
        $date1 = explode("-", $date1);
        $text_date = "วันที่เบิกอุปกรณ์ : " . $date1[0] . " " . $datetime->getTHmonthFromnum($date1[1])  . " " . date('Y');
    } else {
        $date1 = explode("-", $date1);
        $date2 = explode("-", $date2);

        $text_date = "วันที่เบิกอุปกรณ์ : " . $date1[0] . " " . $datetime->getTHmonthFromnum($date1[1]) . " " .date('Y') . " ถึง " .  $date2[0] . " " . $datetime->getTHmonthFromnum($date2[1]) . " " . date('Y');
    }
}
...
// $query = "SELECT
//                 itemslotincabinet_detail.itemcode,
//                 item.itemname,
...
//                 LEFT JOIN item ON itemslotincabinet_detail.itemcode = item.itemcode
//                 LEFT JOIN user_cabinet ON itemslotincabinet_detail.UserID = user_cabinet.User_ID
...
$ddate = date('d_m_Y');
$pdf->Output('Report_log_cabinet_' . $ddate . '.pdf', 'I');
