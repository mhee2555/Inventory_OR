...
$sql = "SELECT-- แยกวันที่ และเวลาออกจาก CreateDate
        DATE_FORMAT( MIN( x.CreateDate ), '%d/%m/%Y' ) AS CreateDate_th,
        DATE_FORMAT( MIN( x.CreateDate ), '%H:%i' ) AS CreateTime,
        x.HnCode,
        x.number_box,
        x.DocNo_SS,-- ชื่อหมอ + หัตถการ (หลายตัวคั่นด้วย , )
        MIN( x.doctor_name ) AS doctor_name,
        MIN( x.procedure_list ) AS procedure_list,-- จำนวนชิ้น
        SUM( x.cnt ) AS total_cnt,-- จ่ายทั้งหมด
        SUM( x.cnt_return ) AS total_return,-- คืนทั้งหมด
        SUM( x.cnt ) - SUM( x.cnt_return ) AS total_use,-- ใช้จริง
        -- % จ่าย / ใช้ / คืน
        100 AS pay_pct,
        CASE
                
                WHEN SUM( x.cnt ) = 0 THEN
                0 ELSE ROUND( ( SUM( x.cnt ) - SUM( x.cnt_return )) / SUM( x.cnt ) * 100, 2 ) 
            END AS use_pct,
        CASE
                
                WHEN SUM( x.cnt ) = 0 THEN
                0 ELSE ROUND( SUM( x.cnt_return ) / SUM( x.cnt ) * 100, 2 ) 
            END AS return_pct,-- ค่าใช้จ่ายรวมคนไข้
            SUM( x.cnt * x.SalePrice ) AS total_price 
        FROM
            (
            SELECT
                hncode.CreateDate,
                ...
                item.SalePrice,
                item.item_status,-- นับจำนวนชิ้น (กันค่า duplicate จากการ join procedure ด้วย DISTINCT)
                COUNT( DISTINCT deproomdetailsub.ID ) AS cnt,-- นับคืน
                (
                SELECT
                    COUNT( lr.id ) 
                FROM
                ...
                WHERE
                    lr.DocNo = hncode.DocNo_SS 
                    AND (
                        ( lr.itemstockID > 0 AND is_return.ItemCode = item.itemcode ) 
                        OR ( lr.itemstockID = 0 AND lr.itemCode = item.itemcode ) 
                    ) 
                ) AS cnt_return,-- ==== เพิ่มหมอและหัตถการที่นี่ ====
                MIN( d.Doctor_Name ) AS doctor_name,-- สมมติชื่อฟิลด์หมอ = doctor.Name
        -- hncode.procedure เก็บเป็น 1,2,3 ให้ใช้ FIND_IN_SET แล้ว GROUP_CONCAT
                GROUP_CONCAT( DISTINCT p.`Procedure_EN` ORDER BY p.`Procedure_EN` SEPARATOR ', ' ) AS procedure_list -- สมมติชื่อฟิลด์ใน procedure = procedure.Name
            ...
            FROM
                ...
                LEFT JOIN itemstock ON deproomdetailsub.ItemStockID IS NOT NULL 
                ...
                ) -- ====== JOIN doctor / procedure เพิ่มเติม ======
                LEFT JOIN doctor d ON d.ID = hncode.doctor
                LEFT JOIN `procedure` p -- ใช้ backtick ครอบเพราะเป็นคำสงวน
...
<tr >
    ...
    <th width="13%" align="center" style="font-size:14px;color:#ffffff;background-color:#663399;">วันที่เข้ารับ<br>บริการ</th>
    ...
    <th width="15%" align="center" style="font-size:14px;color:#ffffff;background-color:#663399;">อาจารย์<br>แพทย์</th>
    ...
    <th width="7%"  align="center" style="font-size:14px;color:#ffffff;background-color:#663399;">ค่าใช้จ่าย<br>คนไข้ (บาท)</th>
</tr>
</thead>';
...
while ($row = $meQuery->fetch(PDO::FETCH_ASSOC)) {
    ...
    // วันที่เข้ารับบริการ
    $html .= '<td width="13%" align="center">' . $row['CreateDate_th'] . '</td>';
    ...
    // อาจารย์แพทย์
    $html .= '<td width="15%" align="center">' . $row['doctor_name'] . '</td>';
    ...
    $html .= '<td width="7%"  align="right">'  . number_format($row['total_price'], 2) . '</td>';
...
<tr nobr="true" style="font-size:13px;">
    <td width="72%" colspan="6" align="center" style="background-color:#D9D9D9;">
